/* MarketGrowth.io - hero fluid canvas.
   Pavel Dobryakov's WebGL fluid simulation (MIT), tuned to the MarketGrowth
   palette and wired to the site theme. Loaded only on pages with a hero canvas. */
/* ============================================================
   The WebGL fluid ink simulation
   (Pavel Dobryakov's WebGL fluid sim, MIT — tuned config,
   strong load burst, cyan→magenta band, perpetual auto-cursor)
   ============================================================ */
function fluidSimulation(canvas) {
    ...
    let config = {
        SIM_RESOLUTION: 200,
        DYE_RESOLUTION: 512,
        DENSITY_DISSIPATION: 0.953, // trails fade so idle swirls stay dynamic, not saturated
        VELOCITY_DISSIPATION: 0.96,
        PRESSURE_DISSIPATION: 0.8,
        PRESSURE_ITERATIONS: 20,
        CURL: 30, // extra swirl for the marbled look
        SPLAT_RADIUS: 0.18, // fatter ink blobs
        SHADING: true,
        COLORFUL: true,
        PAUSED: false,
        BACK_COLOR: backColorForTheme(), // follows the site theme
        TRANSPARENT: false,
        BLOOM: false,
        BLOOM_ITERATIONS: 8,
    ...
    const ORBIT_SPEED = 0.006;     // rad/frame (~17 s per loop — slow, meditative drift)
    const ORBIT_START_DELAY = 700; // ms after load before the auto-cursor begins (burst plays first)
    const ORBIT_STOP_AFTER = 5000; // ms after load when the auto-cursor starts fading out
    const ORBIT_FADE_MS = 2500;    // fade-out duration — ink and stir force ease to zero
    ...
    function update() {
        ...
        input();
        if (!config.PAUSED)
            step(0.009); // slowed simulation timestep for a calmer, drifting fluid
        render(null);
    ...
    function driveVirtualPointer() {
        ...
        if (elapsed > ORBIT_STOP_AFTER + ORBIT_FADE_MS)
            return; // orb has fully faded out — from here only the real pointer stirs the ink
        const orbFade = elapsed <= ORBIT_STOP_AFTER
            ? 1
            : clamp01(1 - (elapsed - ORBIT_STOP_AFTER) / ORBIT_FADE_MS);
        ...
        const dx = (x - vPrevX) * 9.0 * orbFade;
        const dy = (y - vPrevY) * 9.0 * orbFade;
        vPrevX = x;
        vPrevY = y;
        splat(x, y, dx, dy, {
            r: virtualColor.r * orbFade,
            g: virtualColor.g * orbFade,
            b: virtualColor.b * orbFade
        });
    }
    ...
    function generateColor() {
        // MarketGrowth band: teal through blue into violet.
        const h = 0.44 + Math.random() * 0.33;
        const light = config.BACK_COLOR.r > 128;
        let c = HSVtoRGB(h, light ? 0.62 : 0.8, 1.0);
        const k = light ? 0.5 : 0.72;
        c.r *= k;
        c.g *= k;
        c.b *= k;
        return c;
...
function backColorForTheme() {
    var light = document.documentElement.getAttribute('data-theme') === 'light';
    return light ? { r: 244, g: 244, b: 240 } : { r: 6, g: 8, b: 14 };
}

/* Mount the canvas, and rebuild it when the theme changes so the ink
   sits on the right base. Honors reduced-motion by not starting at all. */
(function () {
    ...
    function mount() {
        ...
        try { destroy = fluidSimulation(canvas); } catch (e) { canvas.setAttribute('hidden', ''); }
    }
    mount();
    var pending = null;
